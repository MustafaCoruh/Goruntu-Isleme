...
İlk hedef, canlı RTSP kameraya geçmeden önce lokal geçmiş video ile masa doluluk demosunu doğrulamaktır.

Bu akışta kullanılacak güvenli girişler:

- Lokal config dosyası: `C:\FTMC_FIELD_DATA\configs\tutym2_cam_001.local.json`
- Lokal video: `C:\FTMC_FIELD_DATA\input\videos\`
...
| `Missing local video source` | Video dosyası bulunamadı | `--source` path'i doğru mu kontrol edin |
...
| `Unsupported source extension` | Desteklenmeyen dosya tipi | `.avi`, `.m4v`, `.mkv`, `.mov`, `.mp4` kullanın |
