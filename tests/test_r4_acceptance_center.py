...
def test_r4_acceptance_center_has_terminal_free_acceptance_flow():
    ...
    assert "Güvenli Örnek JSON Dosyalarını İndir" in html
    assert "safeExampleFiles" in html
    assert "ornek_dashboard_durum_raporu.json" in html
    ...
    assert "allRealR4ChecksPassed" in html
    assert "Önce gerçek saha dosyalarıyla üç R4 kontrolü PASS olmalı" in html
